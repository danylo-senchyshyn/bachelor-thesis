...
\begin{enumerate}
    \item hlavná štruktúra práce bola vytvorená podľa štruktúry uvedenej v~článku Beel2010
    \item pre vytvorene abstraktu bola použitá štruktúra IMRaD, ktorá môže byť použitá na napísanie celej práce
\end{enumerate}
...
Táto práca sa venuje komparatívnej analýze monolitickej a mikroservisnej architektúry na konkrétnej backendovej implementácii rezervačného systému. Cieľom bolo prakticky overiť, ako architektonická voľba ovplyvňuje výkon, konzistenciu dát, prevádzkovú náročnosť a udržiavateľnosť systému — nie iba na teoretickej úrovni, ale na základe reálnych meraní a porovnateľných prototypov.

V rámci práce boli vytvorené dve funkčne ekvivalentné implementácie rovnakého systému — monolitická verzia a mikroservisná verzia. Obe pokrývajú rovnakú doménu (správa používateľov, rezervácie, platby, notifikácie) a sú implementované v rovnakom technologickom stacku (Java 17, Spring Boot, PostgreSQL). Mikroservisná verzia navyše využíva Apache Kafka pre asynchrónnu komunikáciu a vzor transactional outbox pre spoľahlivé doručenie udalostí. Tento prístup umožnil objektívne porovnanie dopadov architektonickej voľby bez skreslenia spôsobeného odlišnými technológiami.

Výkonnostné merania pomocou nástroja k6 preukázali, že monolitická architektúra dosahuje výrazne nižšiu end-to-end latenciu (5\,ms oproti 5\,210\,ms) a nižšiu spotrebu zdrojov, zatiaľ čo mikroservisná verzia prináša lepšiu izoláciu zodpovedností, odolnosť voči čiastočným výpadkom a potenciál pre izolované škálovanie. Dominantným faktorom rozdielu v E2E latencii nie je surový výpočtový výkon, ale konfigurácia outbox schedulera — poznatok s priamym dopadom na návrh podobných systémov.

Verím, že táto práca poskytne čitateľom nielen merateľné porovnanie dvoch architektonických prístupov, ale aj praktické poznatky o vzoroch event-driven systémov (saga, transactional outbox, dead letter topic), ktoré sú dnes štandardom pri návrhu distribuovaných backendových aplikácií.
